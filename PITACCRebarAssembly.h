...
namespace PIT
{
	class ACCRebarAssembly : public PITRebarAssembly
	{
	public:
		enum ComponentType
		{
			...
			BE_DATA_VALUE(int, isHandleHole)					//是否规避孔洞
			BE_DATA_VALUE(double, MissHoleSize)					//规避孔洞尺寸
	...
	public:
		...
		ACCRebarAssembly(ElementId id = 0, DgnModelRefP modelRef = NULL) 
...
//		void InitRebarSetId();
	...
	class ACCWallRebarAssembly : public ACCRebarAssembly
	{
	public:
		ACCWallRebarAssembly(ElementId id = 0, DgnModelRefP modelRef = NULL);
		virtual ~ACCWallRebarAssembly() 
		{
...
//		virtual void	CalculateTransform(vector<CVector3D> &vTransform, DgnModelRefP modelRef) {}
	...
	public:
		...
		BE_DECLARE_VMS(ACCSTWallRebarAssembly, RebarAssembly)

	public:
		ACCSTWallRebarAssembly(ElementId id = 0, DgnModelRefP modelRef = NULL) :
			ACCWallRebarAssembly(id, modelRef)
		{
