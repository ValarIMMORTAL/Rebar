...
//获取某一行数据
BOOL ListCtrlEx::CListCtrlEx::GetRowData(int nRow, vector<CString> &oneRowData)
{
	ASSERT(0 <= nRow && nRow < GetItemCount());
	oneRowData.clear();
	for (int nColIndex = 0; nColIndex < GetColumnCount(); ++nColIndex)
	{
		CellData data = m_mapCell2Data[make_pair(nRow, nColIndex)];
		CString strData;
		switch (GetColumnType(nColIndex))
		{
		case Normal:
		case Button:
			strData = GetItemText(nRow, nColIndex);
			break;
		case CheckBox:
			strData.Format(_T("%d"),data.GetCheck());
			break;
		case RadioBox:
			strData.Format(_T("%d"), data.m_nRadioGroup);
			break;
		case ComboBox:
			strData = GetItemText(nRow, nColIndex);
			break;
		case EditBox:
			...
			break;
		case Progress:
			strData.Format(_T("%d"), data.m_uProgresVal);
			break;
		default:
			break;
		}
		oneRowData.push_back(strData);
	}
	return TRUE;
}

//获取所有数据
BOOL ListCtrlEx::CListCtrlEx::GetAllData(map<int, vector<CString> > &mapAllData)
{
	mapAllData.clear();
	for (int nRow = 0; nRow < GetItemCount(); ++ nRow)
	{
		vector<CString> oneRowData;
		GetRowData(nRow, oneRowData);
		mapAllData[nRow] = oneRowData;
	}
	return TRUE;
}
//////////////////////////////////////////////////////////////////////////
										...
										int nRow, int nCol, LRESULT *pResult)
{
	...
	case Button:
		*pResult |= CDRF_SKIPDEFAULT;
		DrawButton(pDC, strText, rcCell, bSelected, FindCellData(nRow, nCol));
		return;
	}
}
...
void CListCtrlEx::DrawButton(CDC * pDC, CString & strText, CRect & rcCell, BOOL bSelected, const CellData & cellData)
{
	COLORREF crText = bSelected ? GetSysColor(COLOR_HIGHLIGHTTEXT) : GetSysColor(COLOR_WINDOWTEXT);
	COLORREF crTextBkgrnd = bSelected ? GetSysColor(COLOR_HIGHLIGHT) : GetSysColor(COLOR_WINDOW);
	COLORREF crBtnBack = bSelected ? GetSysColor(COLOR_HOTLIGHT) : GetSysColor(COLOR_BTNFACE);
	COLORREF crBtnFill = bSelected ? COLOR_FRAME : COLOR_FRAME;
	crBtnBack = cellData.GetEnable() ? crBtnBack : RGB(192, 192, 192);
	crBtnFill = cellData.GetEnable() ? crBtnFill : RGB(192, 192, 192);

	CRect rcBtn(rcCell), rcIcon(rcCell);
	// fill background
	pDC->FillSolidRect(&rcCell, crTextBkgrnd);
	//-- draw icon--//
	CImageList *pImageList = TheImageList();
	...
	rcBtn.left = rcIcon.right + 3;


	pDC->FillSolidRect(&rcBtn, crBtnBack);
	pDC->DrawEdge(&rcBtn, EDGE_SUNKEN, BF_RECT);
	CRect rcFill(rcBtn);
	rcFill.DeflateRect(2, 2, 2, 2);
	rcFill.right = min(rcFill.left + rcBtn.Width(), rcFill.right);
	pDC->FillSolidRect(&rcFill, crBtnFill);
	COLORREF crOldText = pDC->SetTextColor(crText);
	pDC->DrawText(strText, &rcBtn, DT_CENTER);
	pDC->SetTextColor(crOldText);
}
//////////////////////////////////////////////////////////////////////////
// kernel data 
...
void CListCtrlEx::ShowInPlaceCombo(CellIndex ix)
{
	if (ix.first>=0 && ix.first<GetItemCount() && ix.second>=0 && ix.second< GetColumnCount())
	{
		...
		{	
			...
			pInPlaceComboBox->ShowScrollBar(1000);
...
void ListCtrlEx::CListCtrlEx::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (bShouldAction && GetCellEnabled(ix.first, ix.second))
	{
		...
		case Button:
		{
			if ((pWnd = GetParent()) != NULL)
			{
				ButtonCellMsg msg(ix.first, ix.second, true);
				pWnd->SendMessage(WM_ListCtrlEx_BUTTON_LBUTTONDOWN, (WPARAM)this, (LPARAM)&msg);
			}
		}
		break;
		case Normal:
