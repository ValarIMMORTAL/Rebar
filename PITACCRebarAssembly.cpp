...
void ACCRebarAssembly::SetConcreteData(Concrete const& concreteData)
{
	...
	m_isHandleHole = concreteData.isHandleHole;
	m_MissHoleSize = concreteData.MissHoleSize;
}


void ACCRebarAssembly::SetRebarEndTypes(vector<PIT::EndType> const & vecEndTypes)
{
	if (vecEndTypes.size())
		m_vvecEndType.clear();
...
// void ACCRebarAssembly::InitRebarSetId()
// {
// 	int twinRebarLevel = 0;
// 	//根据需求并筋需要设置不一样的层
...
void ACCRebarAssembly::GetConcreteData(Concrete& concreteData)
{
...
void ACCRebarAssembly::SetTieRebarInfo(TieReBarInfo const & tieRebarInfo)
{
	m_tieRebarInfo = tieRebarInfo;
}

const TieReBarInfo ACCRebarAssembly::GetTieRebarInfo() const
{
	return m_tieRebarInfo;
}

ACCRebarAssembly::ComponentType ACCRebarAssembly::JudgeWallType(ElementHandleCR eh)
{
	return STWALL;
	//通过分解元素来判断元素的类型
...
bool ACCRebarAssembly::IsSmartSmartFeature(EditElementHandle& eeh)
{
...
void ACCRebarAssembly::SetRebarData(vector<PIT::ConcreteRebar> const& vecData)
{
...
void ACCRebarAssembly::ClearLines()
{
...
void ACCRebarAssembly::GetRebarData(vector<PIT::ConcreteRebar>& vecData) const
{
	if (0 == GetRebarLevelNum() || GetvecDir().empty() || GetvecDirSize().empty() || GetvecDirSpacing().empty())
	{
		return;
	}
	for (size_t i = 0; i < GetRebarLevelNum(); i++)
	{
		...
		strcpy(rebarData.rebarSize, GetvecDirSize()[i]);
...
ACCWallRebarAssembly::ACCWallRebarAssembly(ElementId id, DgnModelRefP modelRef) :ACCRebarAssembly(id, modelRef)
{

}

bool ACCWallRebarAssembly::IsWallSolid(ElementHandleCR eh)
{
#ifdef PDMSIMPORT
	WString strName;
	WString strType;
...
bool ACCWallRebarAssembly::AnalyzingWallGeometricDataAndHoles(ElementHandleCR eh, STWallGeometryInfo &data)
{
...
bool ACCWallRebarAssembly::AnalyzingWallGeometricData(ElementHandleCR eh, STWallGeometryInfo &data,bool bCut)
{
	double uor_now = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
	double uor_ref = eh.GetModelRef()->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
...
bool ACCSTWallRebarAssembly::makeRebarCurve
(
...
RebarSetTag* ACCSTWallRebarAssembly::MakeRebars
(
...
void ACCSTWallRebarAssembly::CalculateTransform(vector<CVector3D> &vTransform, vector<CVector3D> &vTransformTb,const CVector3D& org,DgnModelRefP modelRef)
{
	if (modelRef == NULL)
		return;
...
void ACCSTWallRebarAssembly::CalculateNowPlacementAndLenth(double &misDisstr, double &misDisend, double& lenth, CMatrix3D   mat, DgnModelRefP modelRef)
{
	if (modelRef == NULL)
		return;
...
void ACCSTWallRebarAssembly::CalculateUseHoles(DgnModelRefP modelRef)
{
	double uor_per_mm = modelRef->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
...
bool ACCSTWallRebarAssembly::MakeRebars(DgnModelRefP modelRef)
{
...
//	for (int j = 0; j < setNum*(iRebarLevelNum-2); ++j)	//新增RebarSetId
	...
	for (int i = 0; i < iRebarLevelNum; ++i)
	{
		...
		double diameter = RebarCode::GetBarDiameter(GetvecDirSize().at(i), modelRef);
		if (COMPARE_VALUES(vecEndType[0].endPtInfo.value1,0) == 0)
		{
			vecEndType[0].endPtInfo.value1 = RebarCode::GetPinRadius(GetvecDirSize().at(i), modelRef, false);
		}
		m_vecRebarPtsLayer.clear();
		if (COMPARE_VALUES(vecEndType[1].endPtInfo.value1, 0) == 0)
		{
			vecEndType[1].endPtInfo.value1 = RebarCode::GetPinRadius(GetvecDirSize().at(i), modelRef, false);
		}
...
// 							}
						...
						PopvecSetId().push_back(0);
						setCount++;
						LineStringRebarMaker lineStrRebar(GetCallerId(),lineSegs, GetvecDirSize().at(i), GetvecDirSpacing().at(i)*uor_per_mm,dWidth-dSideCover*2,dSideCover);
					...
					else
					{
						...
						else //当前层未设置并筋
						{
							...
							PopvecSetId().push_back(0);
							setCount++;
							tag = MakeRebars(PopvecSetId().back(), GetvecDirSize().at(i), dLength, dWidth, GetvecDirSpacing().at(i)*uor_per_mm, GetvecStartOffset().at(i)*uor_per_mm + misDisH_right,
...
long ACCSTWallRebarAssembly::GetStreamMap(BeStreamMap &map, int typeof /* = 0 */, int versionof /* = -1 */)
{
	switch (typeof)
	{
	case 0:
...
bool ACCSTWallRebarAssembly::OnDoubleClick()
{
...
// 	DgnModelRefP modelRef = ACTIVEMODEL;
	...
	pACCSTWallDoubleRebarDlg = new CWallRebarDlg(ehSel, CWnd::FromHandle(MSWIND));
...
bool ACCSTWallRebarAssembly::Rebuild()
{
	if (!GetSelectedModel())
		return false;
...
bool ACCSTWallRebarAssembly::SetComponentData(ElementHandleCR eh)
{
...
void ACCSTWallRebarAssembly::InitUcsMatrix()
{
...
UInt32	ACCSTWallRebarAssembly::JudgeComponentRelation(const STWallGeometryInfo &CurrSTWall, const STWallGeometryInfo &ACCSTwall)
{
	//不与X轴垂直时，左边为起点，右边为终点
	//与X轴垂直时，下边为起点，上边为终点
	//0x00  不与X轴垂直，左上角
