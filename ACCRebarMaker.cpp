...
void CalculateACCUseHoles(DgnModelRefP modelRef, std::vector<EditElementHandle*>& m_Holeehs,std::vector<EditElementHandle*> m_useHoleehs,
	double dSideCover, double misssize, Transform& trans, Transform& matrix,bool isHandleHole)
{
	double uor_per_mm = modelRef->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
	m_useHoleehs.clear();

	if (isHandleHole)//计算需要处理的孔洞
	{
...
ACCRebarMaker::ACCRebarMaker(ElementHandle currEh, const IntersectEle & ACCEh, ACCRebarAssembly * rebarAssembly) 
	: m_eh(currEh), /*m_pACC(new CACC(currEh)),*/ m_ACCEh(ACCEh),m_RebarAssembly(rebarAssembly)
{
	m_associatedRelation = 0;
	...
	DPoint3d ptStart = m_CurSTwallData.ptStart;
	DPoint3d ptEnd = m_CurSTwallData.ptEnd;
	...
	Transform matrix;
	placement.AssignTo(matrix);
	Transform trans;
	placement.AssignTo(trans);
	trans.InverseOf(trans);
	CalculateACCUseHoles(ACTIVEMODEL, m_Holeehs, m_UseHoleehs, rebarAssembly->GetSideCover(),
		rebarAssembly->GetMissHoleSize(), trans, matrix, rebarAssembly->GetisHandleHole());
	GetCoverSideinFrontOrBack();

}

void ACCRebarMaker::AddRebarLevelAndType(RebarElementP rebarElement, BrString sizeKey, DPoint3d ptRebar, DPoint3d ptStr, DPoint3d ptEnd, double dDistance, bool bFlag)
{
	if (rebarElement==nullptr)
	{
		return;
	}
...
bool ACCRebarMaker::ReCalculateWidthAndPoint()
{
	double uor_per_mm = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
	...
	vecFront.Normalize();
	ACCSTWallRebarAssembly *rebarAssembly = dynamic_cast<ACCSTWallRebarAssembly*>(m_RebarAssembly);
	//取当前构件的混凝土保护层
	...
	if (GetCurElementHandle().IsValid())
	{
		...
		curConcrete.concrete.sideCover = rebarAssembly->GetSideCover();
		curConcrete.concrete.postiveCover = rebarAssembly->GetPositiveCover();
		curConcrete.concrete.reverseCover = rebarAssembly->GetReverseCover();
		BrString strTieRebarSize(rebarAssembly->GetTieRebarInfo().rebarSize);
		if (strTieRebarSize != L"" /*&& 0 != GetTieRebarInfo().tieRebarMethod*/)
		{
			CurdiameterTie = RebarCode::GetBarDiameter(strTieRebarSize, ACTIVEMODEL);	//拉筋直径
			CurdiameterTie = CurdiameterTie / UOR_PER_MilliMeter;
		}
	...
	//取关联构件拉筋直径
	double ACCdiameterTie = 0.0;
	ElementId tconcreteid = 0;
	GetElementXAttribute(m_ACCEh.Eh.GetElementId(), sizeof(ElementId), tconcreteid, ConcreteIDXAttribute, m_ACCEh.Eh.GetModelRef());
	TieReBarInfo ACCtieinfo;
	retGetACC = GetElementXAttribute(tconcreteid, sizeof(TieReBarInfo), ACCtieinfo, tieRebarInfoXAttribute, ACTIVEMODEL);
	if (retGetACC != SUCCESS)
	{
		ACCdiameterTie = CurdiameterTie;
	}
	else
	{
		BrString ACCTieRebarSize(ACCtieinfo.rebarSize);
		if (ACCTieRebarSize != L"")
		{
			int TierebarType = ACCtieinfo.rebarType;
			GetDiameterAddType(ACCTieRebarSize, TierebarType);
			ACCdiameterTie = RebarCode::GetBarDiameter(ACCTieRebarSize, ACTIVEMODEL);	//拉筋直径
			ACCdiameterTie = ACCdiameterTie / UOR_PER_MilliMeter;
		}
	}
	double disPosCover = 0;
	double disBkCover = 0;
	disPosCover = ACConcrete.postiveOrTopCover - curConcrete.concrete.sideCover + (ACCdiameterTie - CurdiameterTie);//前面保护层的差值
	disBkCover = ACConcrete.reverseOrBottomCover - curConcrete.concrete.sideCover + (ACCdiameterTie - CurdiameterTie);//后面保护层的差值
	if (0/*disPosCover != 0 || disBkCover != 0*/)
	{
...
bool ACCRebarMaker::ReCalculateWidthAndPointForL4()
{
	double uor_per_mm = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
	DPoint3d midPos = m_CurSTwallData.ptStart;
	midPos.Add(m_CurSTwallData.ptEnd);
	...
	double disPosCover2 = 0;
	double disBkCover2 = 0;
	disPosCover2 = curConcrete.concrete.postiveCover - curConcrete.concrete.sideCover + (ACCdiameterTie - CurdiameterTie);//前面保护层的差值
	disBkCover2 = curConcrete.concrete.reverseCover - curConcrete.concrete.sideCover + (ACCdiameterTie - CurdiameterTie);//后面保护层的差值
	if (disPosCover2 != 0 || disBkCover2 != 0)
	{
		double misDis = (disPosCover2 - disBkCover2)*uor_per_mm / 2;
		vecFront2.Scale(misDis);
...
bool CutWallByACCData(vector<EditElementHandle*> &vecCutWall,vector<DPoint3d>& vecCutPoints, DSegment3dR segLine,ElementHandleCR currEh,double curWallHeight, const vector<IntersectEle> &vecInEle, const vector<PIT::AssociatedComponent>& vecAC, DgnModelRefP modelRef)
{
...
void CutRebarSetByHoles(RebarSetTagArray& rebarsettagas,std::vector<EditElementHandle*>& holes)
{
	double uor_per_mm = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
...
/*
目前整个关联构件的代码逻辑如下：
1、首先定义类似直墙的RebarAssembly指针，ACCSTWallRebarAssembly，然后将计算的正常的配筋参数和模型参数保存在此指针中。CutWallByACCData和SetCutSoild
2、读取所有关联墙体的关联数据，对于每一种关联方式通过ACCRebarMethod1_Maker中的MakeRebars(modelRef)计算主筋的钢筋端部样式和其他一些参数，并保存在ACCRebarMethod1_Maker
定义的指针中。
3、通过ACCSTWallRebarAssembly中的MakeRebar函数将计算得到的所有钢筋集合保存在m_rsetTags中
4、通过ACCRebarMethod1_Maker中的MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)，用来计算相交处的点筋，并将点筋集合保存在ACCSTWallRebarAssembly的m_rsetTags中。
5、通过CutRebarSetByHoles计算钢筋规避孔洞后的钢筋集合
6、将所有钢筋加入到model中	rebarAssembly->AddRebarSets(rebarAssembly->m_rsetTags);
*/
bool ACCRebarMaker::CreateACCRebar(ACCRebarAssembly*  rebar, ElementHandleCR currEh, std::vector<PIT::ConcreteRebar> vecRebarData, DgnModelRefP modelRef)
{
	if (rebar == NULL)
		return NULL;
	bool ret = false;
	CACC acc(currEh);
	vector<ACCRebarMaker*> vecrebarMaker;
	switch (rebar->GetcpType())
	{
	case ACCRebarAssembly::ComponentType::STWALL:
	{
		ACCSTWallRebarAssembly *rebarAssembly = dynamic_cast<ACCSTWallRebarAssembly*>(rebar);
...
//		bool bMakeURebar = false;
		...
		const std::vector<IntersectEle>& vecACCData = acc.GetACCData();
...
//		vecInEle.resize(vecBothEndACCData.size() + vecIntersectSlab.size());
		vecInEle.insert(vecInEle.begin(), vecACCData.begin(), vecACCData.end());
		vecInEle.insert(vecInEle.end(), vecIntersectSlab.begin(), vecIntersectSlab.end());

		vector<EditElementHandle*> vecCutWall;
		DSegment3d segLine;
		bool bRet = CutWallByACCData(vecCutWall, vecCutPts, segLine, currEh, rebarAssembly->GetSTwallData().height, vecInEle, vecAC, modelRef);
		...
		for (size_t i = 0; i < vecInEle.size(); ++i)
		{
			...
			if (it != vecAC.end())
			{
				if ((*it).associatedRelation == 1|| (*it).associatedRelation == 2)	//被锚入
				{
					ACCRebarMaker* rebarMaker = nullptr;
					switch ((*it).anchoringMethod)
					{
					case 0:
 						rebarMaker = new ACCRebarMethod1_Maker(currEh, vecInEle[i], rebar);
						rebarMaker->SetvecRebarData(vecRebarData);
						rebarMaker->m_associatedRelation = (*it).associatedRelation;
 						rebarMaker->MakeRebars(modelRef);
						break;
					case 1:
						rebarMaker = new ACCRebarMethod2_Maker(currEh, vecInEle[i], rebar);
						rebarMaker->m_associatedRelation = (*it).associatedRelation;
						rebarMaker->SetvecRebarData(vecRebarData);
						rebarMaker->MakeRebars(modelRef);
						break;
					case 2:
						rebarMaker = new ACCRebarMethod3_Maker(currEh, vecInEle[i], rebar);
						rebarMaker->m_associatedRelation = (*it).associatedRelation;
						rebarMaker->SetvecRebarData(vecRebarData);
						rebarMaker->MakeRebars(modelRef);
						//					bMakeURebar = true;
						break;
					case 3:
						rebarMaker = new ACCRebarMethod4_Maker(currEh, vecInEle[i], rebar);
						rebarMaker->m_associatedRelation = (*it).associatedRelation;
						rebarMaker->SetvecRebarData(vecRebarData);
						rebarMaker->MakeRebars(modelRef);
						break;
					case 6:
						rebarMaker = new ACCRebarMethod7_Maker(currEh, vecInEle[i], rebar);
						rebarMaker->m_associatedRelation = (*it).associatedRelation;
						rebarMaker->SetvecRebarData(vecRebarData);
						rebarMaker->MakeRebars(modelRef);
						break;
					case 8:
						rebarMaker = new ACCRebarMethod9_Maker(currEh, vecInEle[i], rebar);
						rebarMaker->m_associatedRelation = (*it).associatedRelation;
						rebarMaker->SetvecRebarData(vecRebarData);
						rebarMaker->MakeRebars(modelRef);
						break;
					case 9:
						rebarMaker = new ACCRebarMethod10_Maker(currEh, vecInEle[i], rebar);
						rebarMaker->m_associatedRelation = (*it).associatedRelation;
						rebarMaker->SetvecRebarData(vecRebarData);
						rebarMaker->MakeRebars(modelRef);
						break;
		...
		if (vecrebarMaker.size()>0)
		{
			...
			for (ACCRebarMaker* rebarMaker:vecrebarMaker)
			{
				...
				double uor_per_mm = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
				rebarMaker->m_UseHoleehs.insert(rebarMaker->m_UseHoleehs.end(), rebarAssembly->m_useHoleehs.begin(), rebarAssembly->m_useHoleehs.end());
				CutRebarSetByHoles(rebarAssembly->m_rsetTags, rebarMaker->m_UseHoleehs);
			}	
...
RebarSetTag * ACCRebarMaker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	return NULL;
}

bool ACCRebarMaker::AnalyzingSTWallGeometricData(const ElementHandle & eh,ElementHandle& cureeh, STWallGeometryInfo & stwallData)
{
...
bool ACCRebarMaker::AnalyzingSlabGeometricData(ElementHandleCR eh, ElementHandle& cureeh,double& slabHeight)
{
...
bool ACCRebarMaker::CaluateSTWallIntersectPart(vector<DPoint3d>& vecInSecPartPt)
{
	//不与X轴垂直时，左边为起点，右边为终点
	//与X轴垂直时，下边为起点，上边为终点
	//0x00  不与X轴垂直，左上角
	...
	UInt32 relation = ACCSTWallRebarAssembly::JudgeComponentRelation(m_CurSTwallData, m_InSecSTwallData);
	int iIndex = 0;
	if (relation == 0x000 || relation == 0x100)//左上角和右下角
	{
	...
	else if (relation == 0x001 || relation == 0x101)//右上角
	{
		ptRect[0].Add(vecCur);
		ptRect[0].Add(vecInSecNeg);
	...
	else if (relation == 0x010 || relation == 0x110)//左上和左下
	{
		ptRect[0].Add(vecCurNeg);
...
bool ACCRebarMaker::MakeRebars(DgnModelRefP modelRef)
{
	return false;
}
void ACCRebarMaker::PlusHoles(double dSideCover)
{
	// auto matrix = Transform();
	auto matrix = Transform::FromIdentity();
	for (int j = 0; j < m_Holeehs.size(); j++)
	{
		PlusSideCover(*m_Holeehs.at(j), dSideCover, matrix);
...
bool ACCRebarMaker::GetCoverSideinFrontOrBack()
{
	double uor_now = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
...
bool ACCRebarMaker::makeRebarCurve
(
	DPoint3d ptstr,
	DPoint3d ptend,
...
bool ACCRebarMethod1_Maker::MakeRebars(DgnModelRefP modelRef)
{
	...
	case ACCRebarAssembly::ComponentType::STWALL:
	{
		...
		if (m_ACCEh.Eh.IsValid())	//取关联构件的钢筋尺寸，若关联构件未配筋，则无需绘制交集处纵向钢筋
		{
			...
			if (vecRebar.empty())//关联构件未配置钢筋
			{
				...
				{
					strRebarSize = L"32";	//若没有数据，默认为32mm钢筋
				}
			}
			else
			{
				strRebarSize = vecRebar[0].rebarSize;	//关联构件已配筋则取关联构件第一层钢筋直径
				rebartype = vecRebar[0].rebarType;
			}
		}
		BrString strCurRebarSize(rebarAssembly->GetvecDirSize()[0]);	//当前墙主筋
		/***********************************给sizekey附加型号******************************************************/
		...
		if (strRebarSize.GetLength() > 0)
		{
			if (strRebarSize[strRebarSize.GetLength() - 1] > '9')
			{
				strRebarSize.Delete(strRebarSize.GetLength() - 1);
			}
		}
		if (strCurRebarSize.GetLength() > 0)
		{
			if (strCurRebarSize[strCurRebarSize.GetLength() - 1] > '9')
			{
				strCurRebarSize.Delete(strCurRebarSize.GetLength() - 1);
			}
		}

		int Currebartype = rebarAssembly->GetvecRebarType()[0];
		GetDiameterAddType(strRebarSize, rebartype);
		GetDiameterAddType(strCurRebarSize, Currebartype);
		/***********************************给sizekey附加型号******************************************************/
...
RebarSetTag * ACCRebarMethod1_Maker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
...
bool ACCRebarMethod2_Maker::MakeRebars(DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	bool ret = false;
...
// 		AnalyzingSTWallGeometricData(m_ACCEh.Eh, m_InSecSTwallData);
			...
			if (vecRebar.empty())//关联构件未配置钢筋
			{
				...
				if (vecRebarAC.size())
				{
					...
					if (it != vecRebarAC.end())
					{
						double dim = (*it).endOffset * 0.5;
						char tmpdim[256];
						itoa((int)dim, tmpdim, 10);
						CString sdim(tmpdim);
						sdim = sdim + L"mm";
						if (std::find(g_listRebarSize.begin(), g_listRebarSize.end(),sdim)==g_listRebarSize.end())
						{
							vecRebarsize = 8;
							dim = dim / 2;
						}
		...
		int Currebartype = rebarAssembly->GetvecRebarType()[0];
		if (vecRebar.empty()) rebartype = Currebartype;
		GetDiameterAddType(strRebarSize, rebartype);
		GetDiameterAddType(strCurRebarSize, Currebartype);
		/***********************************给sizekey附加型号******************************************************/
		double diameter = RebarCode::GetBarDiameter(strRebarSize, modelRef);
		...
		{
			CurdiameterTie = RebarCode::GetBarDiameter(strTieRebarSize, modelRef);	//拉筋直径
		}
		//关联构件的混凝土保护层
		ACCConcrete ACConcrete;
		int retGetACC = GetElementXAttribute(m_ACCEh.Eh.GetElementId(), sizeof(ACCConcrete), ACConcrete, ConcreteCoverXAttribute, m_ACCEh.Eh.GetModelRef());
		...
		else
		{
			...
			if (ACCTieRebarSize != L"")
			{
				int TierebarType = ACCtieinfo.rebarType;
				GetDiameterAddType(ACCTieRebarSize, TierebarType);
				ACCdiameterTie = RebarCode::GetBarDiameter(ACCTieRebarSize, modelRef);	//拉筋直径
			}
		}
		...
		if (m_isFrCoverside)//取关联构件的前保护层偏差
		{
			disCover = ACConcrete.postiveOrTopCover - g_wallRebarInfo.concrete.sideCover+ACCdiameterTie/UOR_PER_MilliMeter;
			InSecCover = ACConcrete.postiveOrTopCover + ACCdiameterTie / UOR_PER_MilliMeter;
		}
		else
		{
			disCover = ACConcrete.reverseOrBottomCover - g_wallRebarInfo.concrete.sideCover + ACCdiameterTie / UOR_PER_MilliMeter;
			InSecCover = ACConcrete.reverseOrBottomCover + ACCdiameterTie / UOR_PER_MilliMeter;
		}
		//是否预留长度
		...
		//		RebarAssembly *rebar = ACCRebarAssembly::GetRebarAssembly(concreteid, "class PIT::ACCSTWallRebarAssembly");
		//		if (rebar != nullptr)
		//		{
		//			bReservedlen = false;
		//		}
		...
		if (vvEndType.size())
		{
			...
			switch (m_associatedRelation)
			{
				case 1://当前墙体是锚入墙体,最外侧的钢筋弯曲
				{
					for (size_t i = 0; i < vecRebarData.size(); ++i)
					{
						if (vecRebarData[i].rebarDir == 0)	//横筋才需要处理
						{
							if (level != 0)//如果最外侧钢筋在最前面层
							{
								if (HJnum == 0)//第一次碰到横筋
								{
										vvEndType[i][endTypeIndex].rotateAngle = angle;//端部弯钩方向旋转
										...
										double dRadius = RebarCode::GetPinRadius(strCurRebarSize, modelRef, false) + curDiameter / 2;
										...
										vvEndType[i][endTypeIndex].endPtInfo.value3 = L0Lenth +
											m_CurSTwallData.width - g_wallRebarInfo.concrete.postiveCover*uor_per_mm
											- dRadius + ACConcrete.sideCover * uor_per_mm;								
										HJnum++;
										continue;
								}
								else//第二次或者以上碰到横筋
								{
									if (HJnum == 1 && vecRebarData.size() > 4)//如果钢筋层数大于4，且第二次碰到横筋，还是弯曲
									{
											vvEndType[i][endTypeIndex].rotateAngle = angle;
											vvEndType[i][endTypeIndex].endType = 4;
											vvEndType[i][endTypeIndex].offset -= m_InSecSTwallData.width / uor_per_mm - disCover - 2 * curDiameter / uor_per_mm;
											HJnum = 2;
											double dRadius = RebarCode::GetPinRadius(strCurRebarSize, modelRef, false) + curDiameter / 2 + 2 * curDiameter;
											double L0Lenth = g_globalpara.m_laplenth[(string)strCurRebarSize] * uor_per_mm;
											vvEndType[i][endTypeIndex].endPtInfo.value3 = L0Lenth +
												m_CurSTwallData.width - g_wallRebarInfo.concrete.postiveCover*uor_per_mm
												- dRadius + ACConcrete.sideCover * uor_per_mm;										
									...
									else//如果层数为4时的第二根，或者层数为8时的第三根或者第四根横筋
									{
										vvEndType[i][endTypeIndex].rotateAngle = 0;//端部弯钩方向旋转
										vvEndType[i][endTypeIndex].endType = 0;//端部弯曲方式 90度弯曲 与上边变量一致
										vvEndType[i][endTypeIndex].offset -= m_InSecSTwallData.width / uor_per_mm - disCover - vecRebarData.size() / 2 * curDiameter / uor_per_mm;
										HJnum++;
									}
								}
							}
								...
								else//第二次或者以上碰到横筋
								{
									...
									else//如果层数为4时的第二根，或者层数为8时的第三根或者第四根横筋
									{
										...
										else
										{
											vvEndType[i][endTypeIndex].rotateAngle = angle;//端部弯钩方向旋转
											vvEndType[i][endTypeIndex].endType = 4;//端部弯曲方式 90度弯曲 与上边变量一致
											vvEndType[i][endTypeIndex].offset -= m_InSecSTwallData.width / uor_per_mm - disCover - curDiameter*2 / uor_per_mm;
											//vvEndType[i][endTypeIndex].offset -= (m_InSecSTwallData.width / uor_per_mm -
											//	                                   ACConcrete.postiveOrTopCover + curConcrete.concrete.sideCover);//端点延申
											double dRadius = RebarCode::GetPinRadius(strCurRebarSize, modelRef, false) + curDiameter / 2;
											double L0Lenth = g_globalpara.m_laplenth[(string)strCurRebarSize] * uor_per_mm;
				...
				case 2://当前墙体是被锚入墙体,最外侧的钢筋不变，其他钢筋深入
				{
					for (size_t i = 0; i < vecRebarData.size(); ++i)
					{
						if (vecRebarData[i].rebarDir == 0)	//横筋才需要处理
							...
							if (level != 0)//如果最外侧钢筋在最前面层
							{
								...
								else//第二次或者以上碰到横筋
								{
									...
									else//如果层数为4时的第二根，或者层数为8时的第三根或者第四根横筋
									{
										vvEndType[i][endTypeIndex].rotateAngle = 0;//端部弯钩方向旋转
										vvEndType[i][endTypeIndex].endType = 0;//端部弯曲方式 90度弯曲 与上边变量一致
										vvEndType[i][endTypeIndex].offset -= m_InSecSTwallData.width / uor_per_mm - disCover - vecRebarsize / 2 * diameter / uor_per_mm;
										HJnum++;
									}
								}
							}
...
RebarSetTag * ACCRebarMethod2_Maker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	...
	case ACCRebarAssembly::ComponentType::STWALL:
	{
		...
		GetDiameterAddType(strCurRebarSize, Currebartype);
		/***********************************给sizekey附加型号******************************************************/
		double diameter = RebarCode::GetBarDiameter(strRebarSize, modelRef);//关联构件钢筋直径
		double curDiameter = RebarCode::GetBarDiameter(strCurRebarSize, modelRef);//当前钢筋直径

		double actDiameter = diameter > curDiameter ? diameter : curDiameter;
		BrString strActRebarSize = diameter > curDiameter ? strRebarSize : strCurRebarSize;

		//默认计算得到的四个点中第一个点为最外侧的交点
		DVec3d vecX = m_vecInSecPartPt[1] - m_vecInSecPartPt[0];
		DVec3d vecY = m_vecInSecPartPt[2] - m_vecInSecPartPt[1];
		double XLenth = m_vecInSecPartPt[1].Distance(m_vecInSecPartPt[0]);
		double YLenth = m_vecInSecPartPt[2].Distance(m_vecInSecPartPt[1]);
		vecX.Normalize();
		vecY.Normalize();
		double XsideCover = 0;
		double YsideCover = 0;
		double Xdiameter = 0;
		double Ydiameter = 0;
		int Xnum = 0;//X方向横向钢筋根数
		int Ynum = 0;//Y方向横向钢筋根数
		DVec3d vecCur = m_CurSTwallData.ptEnd - m_CurSTwallData.ptStart;
		vecCur.Normalize();
		{//计算X方向的和Y方向的直径和间距以及钢筋层数
			
			if (abs(vecCur.DotProduct(vecX))>0.99)//当前墙体为X方向，关联墙体为Y方向
			{
				XsideCover = (rebarAssembly->GetPositiveCover()+rebarAssembly->GetReverseCover()+CurdiameterTie*2)/2*uor_per_mm;
				Xdiameter = curDiameter;
				YsideCover = (ACConcrete.postiveOrTopCover + ACConcrete.reverseOrBottomCover+ACCdiameterTie*2) / 2 * uor_per_mm;
				Ydiameter = diameter;
				Xnum = (int)(rebarAssembly->GetvecDirSize().size()) / 2;
				Ynum = (int)(vecRebarsize) / 2;
			}
			else
			{
				DPoint3d tmppt = m_vecInSecPartPt[1];
				m_vecInSecPartPt[1] = m_vecInSecPartPt[3];
				m_vecInSecPartPt[3] = tmppt;
				vecX = m_vecInSecPartPt[1] - m_vecInSecPartPt[0];
				vecY = m_vecInSecPartPt[2] - m_vecInSecPartPt[1];
				XLenth = m_vecInSecPartPt[1].Distance(m_vecInSecPartPt[0]);
				YLenth = m_vecInSecPartPt[2].Distance(m_vecInSecPartPt[1]);
				XsideCover = (rebarAssembly->GetPositiveCover() + rebarAssembly->GetReverseCover() + CurdiameterTie * 2) / 2 * uor_per_mm;
				Xdiameter = curDiameter;
				YsideCover = (ACConcrete.postiveOrTopCover + ACConcrete.reverseOrBottomCover + ACCdiameterTie * 2) / 2 * uor_per_mm;
				Ydiameter = diameter;
		...
		DVec3d vecA = vecX;
		DVec3d vecB = vecY;
		double moveOffsetX = actDiameter + (1 - 0.866025)*0.707106*actDiameter/2;//(1-cos30)*sin45*radius
		double moveOffsetY = actDiameter + (1 - 0.866025)*0.707106*actDiameter/2;
		vecA.ScaleToLength(moveOffsetX);
		vecB.ScaleToLength(moveOffsetY);
		...
		int numRebar = 0;
		double spacing = 200 * uor_per_mm;
		adjustedXLen = XLenth - YsideCover * 2 - 2*actDiameter - Ydiameter;
		int numRebarX = (int)floor(adjustedXLen / spacing + 0.5) + 1;
		adjustedXSpacing = spacing;
		if (numRebarX > 1)
			adjustedXSpacing = adjustedXLen / (numRebarX - 1);

		adjustedYLen =YLenth - XsideCover * 2 - 2 * actDiameter - Xdiameter;
		int numRebarY = (int)floor(adjustedYLen / spacing + 0.5) + 1;
		adjustedYSpacing = spacing;
		if (numRebarY > 1)
			adjustedYSpacing = adjustedYLen / (numRebarY - 1);
		...
		vector<DSegment3d> vecRebarPt;
		double dOffsetX = YsideCover + 0.5*actDiameter + actDiameter;
		double dOffsetY = XsideCover + 0.5*actDiameter + actDiameter;
		vecY.Scale(dOffsetY);
		DPoint3d ptOrign = m_vecInSecPartPt[0];
		ptOrign.Add(vecY);
		DVec3d vec1 = vecX;
		vec1.Normalize();
		double dOffset = dOffsetX;
		for (int i = 0; i < numRebarX; ++i)
		{
			PITRebarCurve rebarcurve;
			vec1.ScaleToLength(dOffset);
			...
			if (Xnum > 2 || Ynum > 2)
			{
				DVec3d vecA2 = vecX;
				DVec3d vecB2 = vecY;
				vecA2.ScaleToLength(actDiameter * 2);
				vecB2.ScaleToLength(actDiameter * 2);
				DPoint3d ptStart2 = ptStart;
				if (i==0)
				{
					ptStart2.Add(vecA2);
					ptStart2.Add(vecB2);
				}
				else if(i==numRebarX-1)
				{
					if (Ynum > 2)
					{
						vecA2.Scale(-1);
				...
				ptStart2.z += sideCover;
				DPoint3d ptEnd2 = ptStart2;
				ptEnd2.z += m_CurSTwallData.height;
				ptEnd2.z -= sideCover * 2;
				vecRebarPt.push_back(DSegment3d::From(ptStart2, ptEnd2));
			}
			ptStart.z += sideCover;
			DPoint3d ptEnd = ptStart;
			ptEnd.z += m_CurSTwallData.height;
		...
		for (int i = 0; i < numRebarY - 1; ++i)
		{
			...
			if (Ynum>2)
			{
				...
				DPoint3d ptEnd2 = ptStart2;
				ptEnd2.z = vecRebarPt.back().point[1].z;
				vecRebarPt.push_back(DSegment3d::From(ptStart2, ptEnd2));
			}
			else if (Xnum>2&&i== numRebarY - 2)
			{
				DVec3d vecB2 = vecY;
				vecB2.ScaleToLength(-actDiameter * 2);
				DPoint3d ptStart2 = ptStart;
...
RebarSetP ACCRebarMethod3_Maker::MakeURebarCurve(ElementId rebarSetId, const vector<vector<DSegment3d> >& vvecStartEnd,double width, DgnModelRefP modelRef)
{
	double uor_per_mm = modelRef->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
	...
	BrString rebarSize = m_RebarAssembly->PopvecDirSize().at(0);
	...
	ACCSTWallRebarAssembly *rebarAssembly = nullptr;
	switch (m_RebarAssembly->GetcpType())
	{
	  case ACCRebarAssembly::ComponentType::STWALL:
	  {
		rebarAssembly = dynamic_cast<ACCSTWallRebarAssembly*>(m_RebarAssembly);
	   }
	}
	...
	size_t dURebarNumber = vecRebarCurve.size(); // U型筋数量
	m_RebarAssembly->SetSideCover(tmpSidecover);
...
bool ACCRebarMethod3_Maker::MakeIntersectionRebarCurve(std::vector<PITRebarCurve>& vecRebarCurve, DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return false;
...
bool ACCRebarMethod3_Maker::MakeRebars(DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	bool ret = false;
...
// 		AnalyzingSTWallGeometricData(m_ACCEh.Eh, m_InSecSTwallData);
		...
		double angle = 0;
		vector<vector<PIT::EndType> > &vvEndType = rebarAssembly->PopvvecEndType();
		m_Relation = ACCSTWallRebarAssembly::JudgeComponentRelation(m_CurSTwallData, m_InSecSTwallData);
...
RebarSetTag * ACCRebarMethod3_Maker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	if (modelRef == NULL)
		return NULL;
...
bool ACCRebarMethod4_Maker::MakeRebars(DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	bool ret = false;
...
// 		AnalyzingSTWallGeometricData(m_ACCEh.Eh, m_InSecSTwallData);
		...
		if (vvEndType.size())
		{
			...
			if (m_isFrCoverside)
			{
				dOffset += ACConcrete.postiveOrTopCover + ACCdiameterTie - g_wallRebarInfo.concrete.sideCover;
			}
			else
			{
				dOffset += ACConcrete.reverseOrBottomCover + ACCdiameterTie - g_wallRebarInfo.concrete.sideCover;
			}
			for (size_t i = 0; i < vecRebarData.size(); ++i)
			{
				if (vecRebarData[i].rebarDir == 0)	//所有横向钢筋都弯曲
				{
					...
					vvEndType[i][endTypeIndex].endType = 0;			//端部弯曲方式 90度弯曲 与上边变量一致
					double dRadius = RebarCode::GetPinRadius(vecRebarData[i].rebarSize, modelRef, false);
					double dim = RebarCode::GetBarDiameter(vecRebarData[i].rebarSize, modelRef);
					...
					double LaLenth = g_globalpara.m_alength[vecRebarData[i].rebarSize] * uor_per_mm;
...
RebarSetTag * ACCRebarMethod4_Maker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	...
	case ACCRebarAssembly::ComponentType::STWALL:
	{
		...
		if (1)//不管是4层钢筋还是8层钢筋都要走这个操作
		{
			adjustedXLen = m_InSecSTwallData.width - (ACConcrete.postiveOrTopCover + ACConcrete.reverseOrBottomCover+ACCdiameterTie*2)*uor_per_mm - diameter * 2 /*(vecRebar.size()-2)*/ - actDiameter;
			int numRebarX = (int)floor(adjustedXLen / spacing + 0.5) + 1;
			if (numRebarX == 0)
			{
				return tag;
			...
			adjustedYLen = m_CurSTwallData.width - (rebarAssembly->GetPositiveCover() + rebarAssembly->GetReverseCover()+CurdiameterTie*2)*uor_per_mm - curDiameter * 2 /*(GetvecRebarData().size()-2)*/ - actDiameter;
			int numRebarY = (int)floor(adjustedYLen / spacing + 0.5) + 1;
			adjustedYSpacing = spacing;
			if (numRebarY > 1)
				adjustedYSpacing = adjustedYLen / (numRebarY - 1);
			...
			double dOffset = actDiameter / 2 + curDiameter + (rebarAssembly->GetPositiveCover() + rebarAssembly->GetReverseCover()+ CurdiameterTie * 2)*uor_per_mm / 2;
			DVec3d vecY = vec2;
			vecY.Scale(dOffset);
			dOffset = actDiameter / 2 + diameter  + (ACConcrete.postiveOrTopCover + ACConcrete.reverseOrBottomCover+ ACCdiameterTie * 2)*uor_per_mm / 2;
			DPoint3d ptOrign = m_vecInSecPartPt[0];
			ptOrign.Add(vecY);
			for (int i = 0; i < numRebarX; ++i)
			{
		...
		if (GetvecRebarData().size()>4||vecRebar.size()>4)//当前钢筋层数大于4时
		{
			vec1 = m_vecInSecPartPt[1] - m_vecInSecPartPt[0];
			vec2 = m_vecInSecPartPt[2] - m_vecInSecPartPt[1];
			vec1.Normalize();
			vec2.Normalize();
			adjustedXLen = m_InSecSTwallData.width - (ACConcrete.postiveOrTopCover + ACConcrete.reverseOrBottomCover + ACCdiameterTie * 2)*uor_per_mm - diameter * (vecRebar.size()-2) - actDiameter;
			int numRebarX = (int)floor(adjustedXLen / spacing + 0.5) + 1;
			if (numRebarX == 0)
			{
				return tag;
			...
			adjustedYLen = m_CurSTwallData.width - (rebarAssembly->GetPositiveCover() + rebarAssembly->GetReverseCover() + CurdiameterTie * 2)*uor_per_mm - curDiameter * (GetvecRebarData().size()-2) - actDiameter;
			int numRebarY = (int)floor(adjustedYLen / spacing + 0.5) + 1;
			adjustedYSpacing = spacing;
			if (numRebarY > 1)
				adjustedYSpacing = adjustedYLen / (numRebarY - 1);
			...
			double dOffset = actDiameter / 2 + curDiameter*(GetvecRebarData().size()/2-1)+ (rebarAssembly->GetPositiveCover() + rebarAssembly->GetReverseCover() + CurdiameterTie * 2)*uor_per_mm / 2;
			DVec3d vecY = vec2;
			vecY.Scale(dOffset);
			dOffset = actDiameter / 2 + diameter* (vecRebar.size()/2 - 1) + (ACConcrete.postiveOrTopCover + ACConcrete.reverseOrBottomCover + ACCdiameterTie * 2)*uor_per_mm / 2;
			DPoint3d ptOrign = m_vecInSecPartPt[0];
			ptOrign.Add(vecY);
			for (int i = 0; i < numRebarX; ++i)
			{
...
bool ACCRebarMethod7_Maker::MakeRebars(DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	bool ret = false;
...
RebarSetTag * ACCRebarMethod7_Maker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	return nullptr;
}

bool ACCRebarMethod9_Maker::MakeRebars(DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	bool ret = false;
...
RebarSetTag * ACCRebarMethod9_Maker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	return nullptr;
}

bool ACCRebarMethod10_Maker::MakeRebars(DgnModelRefP modelRef)
{
	if (m_RebarAssembly == NULL)
		return NULL;
	bool ret = false;
...
RebarSetTag * ACCRebarMethod10_Maker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	return nullptr;
}

RebarSetTag * LineStringRebarMaker::MakeRebar(ElementId rebarSetId, DgnModelRefP modelRef)
{
	if (modelRef == NULL)
		return NULL;
...
bool LineStringRebarMaker::makeRebarCurve
(
	DPoint3d ptstr,
	DPoint3d ptend,
...
RebarSetP LineStringRebarMaker::MakeLineStringRebar(ElementId rebarSetId, ElementId callerId, const vector<LineSegment>& segs, double offset, DgnModelRefP modelRef)
{
	if (segs.empty())
	{
		return nullptr;
