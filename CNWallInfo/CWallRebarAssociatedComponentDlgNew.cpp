...
IMPLEMENT_DYNAMIC(CWallRebarAssociatedComponentDlgNew, CDialogEx)

CWallRebarAssociatedComponentDlgNew::CWallRebarAssociatedComponentDlgNew(ElementHandleCR eeh, CWnd * pParent)
	: CDialogEx(IDD_DIALOG_CNWallRebar_AssociatedComponent, pParent),m_ehSel(eeh),m_pACC(new CAssociatedComponent(eeh))
{
	InitACCData();
}

CWallRebarAssociatedComponentDlgNew::~CWallRebarAssociatedComponentDlgNew()
{
	
}

void CWallRebarAssociatedComponentDlgNew::InitACCData()
{
	if (m_vecAC.size() > 0)
	{
		return;
...
//	m_ACCData.insert(m_ACCData.end(), vecIntersectSlab.begin(), vecIntersectSlab.end());
	...
	for (size_t i = 0; i < Same_Eles.InSDatas.size(); ++i)
	{
		...
		if (vecRebarInfo.size() == 4)
		{
			WString strSize0(vecRebarInfo[0].rebarSize);
			WString strSize1(vecRebarInfo[1].rebarSize);
			if (strSize0.find(L"mm") != WString::npos)
				strSize0.ReplaceAll(L"mm", L"");
			if (strSize1.find(L"mm") != WString::npos)
				strSize1.ReplaceAll(L"mm", L"");

			oneACData.endOffset = atof(CT2A(strSize0.c_str())) + atof(CT2A(strSize1.c_str()));
		}
		//end
...
void CWallRebarAssociatedComponentDlgNew::DoDataExchange(CDataExchange* pDX)
{
...
void CWallRebarAssociatedComponentDlgNew::InitUIData()
{
...
void CWallRebarAssociatedComponentDlgNew::SetListDefaultData()
{
}

void CWallRebarAssociatedComponentDlgNew::UpdateACList()
{
...
BEGIN_MESSAGE_MAP(CWallRebarAssociatedComponentDlgNew, CDialogEx)
	ON_MESSAGE(WM_ListCtrlEx_BUTTON_LBUTTONDOWN, &CWallRebarAssociatedComponentDlgNew::OnACCButtonDown)
	ON_MESSAGE(WM_ListCtrlEx_LBUTTONDOWN, &CWallRebarAssociatedComponentDlgNew::OnACCCtrlListLButtonDown)
	ON_WM_DESTROY()
	ON_WM_CLOSE()
END_MESSAGE_MAP()
...
BOOL CWallRebarAssociatedComponentDlgNew::OnInitDialog()
{
...
LRESULT CWallRebarAssociatedComponentDlgNew::OnACCButtonDown(WPARAM wParam, LPARAM lParam)
{
	...
	if (it != m_ACCData.end())
	{
		...
		CACCDataSetDlgNew accdlg((*it).Eh, wallACConcrete, m_vecAC[msg->m_nRow].endOffset, this);	//使用默认值50
...
LRESULT CWallRebarAssociatedComponentDlgNew::OnACCCtrlListLButtonDown(WPARAM wParam, LPARAM lParam)
{
...
void CWallRebarAssociatedComponentDlgNew::OnDestroy()
{
...
void CWallRebarAssociatedComponentDlgNew::OnClose()
{
