...
bool CAssociatedComponentNew::GetComponentCenterLine(ElementHandleCR eeh, vector<DPoint3d> &vecVertex,double *fuzzyThickness)
{
...
void CAssociatedComponentNew::GetAllIntersectDatas()
{
	...
	{
		if (tmphd.EleName.find("EB") == string::npos && tmphd.EleName.find("NB") == string::npos && tmphd.EleName.find("DB") == string::npos)
...
//			m_InSSlabDatas.push_back(tmphd);
	...
	{
		if (tmphd.EleName.find("EB") == string::npos && tmphd.EleName.find("NB") == string::npos)
			m_InSDatas.push_back(tmphd);
		if (tmphd.EleName.find("DB") != string::npos)
			m_InSSlabDatas.push_back(tmphd);
	}
...
void CAssociatedComponentNew::GetBothEndIntersectDatas()
{
	if (!m_InSDatas.size())
