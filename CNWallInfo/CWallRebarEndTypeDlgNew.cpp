...
IMPLEMENT_DYNAMIC(CWallRebarEndTypeDlgNew, CDialogEx)

CWallRebarEndTypeDlgNew::CWallRebarEndTypeDlgNew(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DIALOG_CNWallRebar_EndType, pParent)
{

}

CWallRebarEndTypeDlgNew::~CWallRebarEndTypeDlgNew()
{
}

void CWallRebarEndTypeDlgNew::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST_EndType, m_ListEndType);
}

void CWallRebarEndTypeDlgNew::InitUIData()
{
	LONG lStyle;
...
void CWallRebarEndTypeDlgNew::SetListDefaultData()
{
...
void CWallRebarEndTypeDlgNew::UpdateEndTypeList()
{
...
BEGIN_MESSAGE_MAP(CWallRebarEndTypeDlgNew, CDialogEx)
	ON_MESSAGE(WM_ListCtrlEx_BUTTON_LBUTTONDOWN, &CWallRebarEndTypeDlgNew::OnEndTypeButtonDown)
END_MESSAGE_MAP()
...
BOOL CWallRebarEndTypeDlgNew::OnInitDialog()
{
...
LRESULT CWallRebarEndTypeDlgNew::OnEndTypeButtonDown(WPARAM wParam, LPARAM lParam)
{
	...
	if (2 == msg->m_nColumn)
	{
		...
		BrString sizekey = XmlManager::s_alltypes[m_vecRebarData[msg->m_nRow / 2].rebarType];

		dlg.SetRebarSize(sizekey);
		if (IDOK == dlg.DoModal())
