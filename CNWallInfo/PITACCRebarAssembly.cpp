...
#include "./MFC/CWallRebarDlgNew.h"
...
void ACCCNRebarAssembly::SetConcreteData(Concrete const& concreteData)
{
...
void ACCCNRebarAssembly::SetRebarEndTypes(vector<PIT::EndType> const & vecEndTypes)
{
	if (vecEndTypes.size())
		m_vvecEndType.clear();
...
void ACCCNRebarAssembly::GetConcreteData(Concrete& concreteData)
{
...
void ACCCNRebarAssembly::SetTieRebarInfo(TieReBarInfo const & tieRebarInfo)
{
	m_tieRebarInfo = tieRebarInfo;
}

const TieReBarInfo ACCCNRebarAssembly::GetTieRebarInfo() const
{
	return m_tieRebarInfo;
}

ACCCNRebarAssembly::ComponentType ACCCNRebarAssembly::JudgeWallType(ElementHandleCR eh)
{
	return STWALL;
	//通过分解元素来判断元素的类型
...
bool ACCCNRebarAssembly::IsSmartSmartFeature(EditElementHandle& eeh)
{
...
void ACCCNRebarAssembly::SetRebarData(vector<PIT::ConcreteRebar> const& vecData)
{
...
void ACCCNRebarAssembly::ClearLines()
{
...
void ACCCNRebarAssembly::GetRebarData(vector<PIT::ConcreteRebar>& vecData) const
{
...
bool ACCCNWallRebarAssembly::IsWallSolid(ElementHandleCR eh)
{
#ifdef PDMSIMPORT
	WString strName;
	WString strType;
...
bool ACCCNWallRebarAssembly::AnalyzingWallGeometricDataAndHoles(ElementHandleCR eh, STWallGeometryInfo &data)
{
...
bool ACCCNWallRebarAssembly::AnalyzingWallGeometricData(ElementHandleCR eh, STWallGeometryInfo &data,bool bCut)
{
	double uor_now = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
	double uor_ref = eh.GetModelRef()->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
...
ACCCNSTWallRebarAssembly::ACCCNSTWallRebarAssembly(ElementId id, DgnModelRefP modelRef) :ACCCNWallRebarAssembly(id, modelRef)
{
	;
}

bool ACCCNSTWallRebarAssembly::makeRebarCurve
(
...
RebarSetTag* ACCCNSTWallRebarAssembly::MakeRebars
(
...
void ACCCNSTWallRebarAssembly::CalculateTransform(vector<CVector3D> &vTransform, vector<CVector3D> &vTransformTb,const CVector3D& org,DgnModelRefP modelRef)
{
	if (modelRef == NULL)
		return;
...
void ACCCNSTWallRebarAssembly::CalculateNowPlacementAndLenth(double &misDisstr, double &misDisend, double& lenth, CMatrix3D   mat, DgnModelRefP modelRef)
{
	if (modelRef == NULL)
		return;
...
void ACCCNSTWallRebarAssembly::CalculateUseHoles(DgnModelRefP modelRef)
{
	double uor_per_mm = modelRef->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
...
bool ACCCNSTWallRebarAssembly::MakeRebars(DgnModelRefP modelRef)
{
...
long ACCCNSTWallRebarAssembly::GetStreamMap(BeStreamMap &map, int typeof /* = 0 */, int versionof /* = -1 */)
{
	switch (typeof)
	{
	case 0:
...
bool ACCCNSTWallRebarAssembly::OnDoubleClick()
{
...
bool ACCCNSTWallRebarAssembly::Rebuild()
{
	if (!GetSelectedModel())
		return false;
...
bool ACCCNSTWallRebarAssembly::SetComponentData(ElementHandleCR eh)
{
...
void ACCCNSTWallRebarAssembly::InitUcsMatrix()
{
...
UInt32	ACCCNSTWallRebarAssembly::JudgeComponentRelation(const STWallGeometryInfo &CurrSTWall, const STWallGeometryInfo &ACCSTwall)
{
	//不与X轴垂直时，左边为起点，右边为终点
	//与X轴垂直时，下边为起点，上边为终点
	//0x00  不与X轴垂直，左上角
