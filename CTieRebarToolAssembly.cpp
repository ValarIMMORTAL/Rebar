...
CTieRebarToolAssembly::CTieRebarToolAssembly(ElementId id, DgnModelRefP modelRef)
{
	InitReabrData();
}

CTieRebarToolAssembly::~CTieRebarToolAssembly()
{
...
void CTieRebarToolAssembly::InitReabrData()
{
	m_MainFirLevSizekey = "";
	m_MainSecLevSizekey = "";
...
void CTieRebarToolAssembly::GetEleNameAndType(ElementHandleR eeh)
{
	WString Ename;
	WString Etype;
...
bool CTieRebarToolAssembly::AnalyzingWallGeometricData(ElementHandleCR eh)
{
...
bool CTieRebarToolAssembly::IsSmartSmartFeature(EditElementHandle& eeh)
{
...
bool CTieRebarToolAssembly::push_vecPoint(vector<DSegment3d>& vecLevRebar, DPoint3d& ptstr, DPoint3d& ptend, CVector3D& vec, CVector3D& vecAnthor, DgnModelRefP modelRef)
{
	bool bFlag = false;
...
void CTieRebarToolAssembly::SortVecRebar(vector<DSegment3d>& vecFirVec, vector<DSegment3d>& vecSecVec)
{
	DPoint3d ptFirStr, ptFirEnd, ptStr;
	DPoint3d ptSecStr, ptSecEnd;
	vecFirVec[0].GetStartPoint(ptFirStr);
	vecFirVec[0].GetEndPoint(ptFirEnd);
	vecSecVec[0].GetStartPoint(ptSecStr);
	vecSecVec[0].GetEndPoint(ptSecEnd);

	CVector3D vec = ptFirEnd - ptFirStr; // 第一层方向
	vec.Normalize();
	CVector3D vecNormol = CVector3D::kZaxis.CrossProduct(vec);
	if (vec.IsEqual(CVector3D::kZaxis, EPS))
	{
		vecNormol = CVector3D::kYaxis.CrossProduct(vec);
	}

	// 计算一个点在由原点和法线 bvector 定义的平面上的投影 ptStr 投影点
	mdlVec_projectPointToPlane(&ptStr, &ptSecStr, &ptFirStr, &vecNormol);

	CVector3D vecCom = m_ptEnd - m_ptStart;
	vecCom.Normalize();
	CVector3D vecTmp = vec;
	if (!vecTmp.IsEqual(vecCom, EPS))
	{
		vecTmp = ptSecEnd - ptSecStr;
		vecTmp.Normalize();
	}
	CVector3D vecIndex = vecCom.CrossProduct(vecTmp);

	vec = ptStr - ptFirStr;
	vec.Normalize();
	if (!vec.IsEqual(vecIndex, EPS))
	{
		vector<DSegment3d> vecBack;
		vecBack.insert(vecBack.begin(), vecFirVec.begin(), vecFirVec.end());
		vecFirVec.clear();
		vecFirVec.insert(vecFirVec.begin(), vecSecVec.begin(), vecSecVec.end());
		m_vecAnthorFirLevRebar.clear();
		m_vecAnthorFirLevRebar.insert(m_vecAnthorFirLevRebar.begin(), vecBack.begin(), vecBack.end());
		vecBack.clear();
	}
}

void CTieRebarToolAssembly::SortVecRebar(vector<DSegment3d>& vecSeg, const CVector3D& perpendicularSortVec, const CVector3D& parallelSortVec)
{
	//先在钢筋线垂直方向做排序
...
void CTieRebarToolAssembly::SortAllVecRebar()
{
	// 钢筋按照指定方向排序 如按X轴正向或垂直Z方向等
	//SortVecRebar(m_vecMainFirLevRebar, m_AnthorLevVec);
	//SortVecRebar(m_vecMainSecLevRebar, m_AnthorLevVec);
	//SortVecRebar(m_vecAnthorFirLevRebar, m_MainLevVec);
	//SortVecRebar(m_vecAnthorSecLevRebar, m_MainLevVec);
	...
	//SortVecRebar(m_vecMainFirLevRebar, m_vecAnthorFirLevRebar);
	//SortVecRebar(m_vecMainSecLevRebar, m_vecAnthorSecLevRebar);

	//SortVecRebar(m_vecMainFirLevRebar, m_vecMainSecLevRebar);
	//SortVecRebar(m_vecAnthorFirLevRebar, m_vecAnthorSecLevRebar);
}

// 计算已选择的钢筋信息
void CTieRebarToolAssembly::CalculateSelectRebarInfo(vector<ElementRefP>& vecSelectrebars, DgnModelRefP modelRef)
{
	...
	for (vector<ElementRefP>::iterator itr = vecSelectrebars.begin(); itr != vecSelectrebars.end(); itr++)
	{
		...
		if (RebarElement::IsRebarElement(eehHandle))
		{
			...
			if (m_MainLevVec.IsZero() || !m_MainLevVec.IsPerpendicularTo(ptend - ptstr)) // 主方向
			{
				// 第一层
...
void CTieRebarToolAssembly::CalaRebarStartEnd(RebarElementP rep, DPoint3d& PtStar, DPoint3d& PtEnd, double& diameter, DgnModelRefP modelRef)
{
	RebarCurve curve;
	double uor_per_mm = ACTIVEMODEL->GetModelInfoCP()->GetUorPerMeter() / 1000.0;
...
// 选择连续配筋或只选择了一个方向的钢筋时，选要遍历与之有相交的钢筋
void CTieRebarToolAssembly::TraveAllRebar(RebarAssembly* pRebarAssembly, DgnModelRefP modelRef)
{
...
void CTieRebarToolAssembly::SortVecRebar(vector<DSegment3d>& vecSeg, const CVector3D& sortVec)
{
...
// 取钢筋起始点
void CTieRebarToolAssembly::GetStartEndPoint(vector<vector<DSegment3d> >& vecStartEnd)
{
	if (m_vecMainFirLevRebar.size() == 0 || m_vecMainSecLevRebar.size() == 0 ||
		m_vecAnthorFirLevRebar.size() == 0 || m_vecAnthorSecLevRebar.size() == 0)
	{
...
bool CTieRebarToolAssembly::MakeRebars(DgnModelRefP modelRef)
{
	...
	// 配置间隔方式，如间隔开一根或几根
	TieRebarMaker tieRebarMaker(faceDataArray, vecStartEnd, (TieRebarStyle)m_tieRebarInfo.tieRebarStyle, strTieRebarSize);
...
void CTieRebarToolAssembly::GetHoleWithElement(ElementHandleCR eh)
{
	DPoint3d FrontStr, FrontEnd;
	DPoint3d BackStr, BackEnd;
	std::vector<EditElementHandle*> Negs;
...
long CTieRebarToolAssembly::GetStreamMap(BeStreamMap &map, int typeof /* = 0 */, int versionof /* = -1 */)
{
	switch (typeof)
	{
	case 0:
