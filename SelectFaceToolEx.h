...
/*=================================================================================**//**
* 示例显示如何使用定位子实体工具编写用于应用的工具
* 使用固体内核API对面的本地操作。
*
* @bsiclass                                                               Bentley Systems
+===============+===============+===============+===============+===============+======*/
struct          SelectFaceToolEx : LocateSubEntityTool
{
	ElementHandle	_eh;
	ElementId		_ehNew;
	int				_selectNum;
public:
	SelectFaceToolEx(int cmdName, int prompt, ElementHandleCR ehOld, ElementId ehNew, int selectNum = 1) :_eh(ehOld), _ehNew(ehNew), _selectNum(selectNum)
	{
...
// 	/*---------------------------------------------------------------------------------**//**
// 	* 创建和安装工具的新实例的方法。如果安装工具返回错误，
// 	*新的工具实例将被释放/无效。切勿在裁判计数类中调用DELETE。
// 	* @bsimethod                                                          Bentley Systems
// 	+---------------+---------------+---------------+---------------+---------------+------*/
...
#pragma endregion 参数化创建墙和孔洞
				...
				SelectFaceToolEx* tool = new SelectFaceToolEx(toolId, prompt, eeh, ehNew);
				tool->InstallTool();
			}
			else
			{
...
#pragma endregion 参数化创建墙和孔洞
				...
				//mdlInput_sendSynchronizedKeyin(L"reference display off all", 0, INPUTQ_EOQ, NULL);
				mdlInput_sendSynchronizedKeyin(L"displayset set selection", 0, INPUTQ_HEAD, NULL);
				SelectFaceToolEx* tool = new SelectFaceToolEx(toolId, prompt, testeeh, ehNew);
				tool->InstallTool();
			}
