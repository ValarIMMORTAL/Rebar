...
class CFacesRebarDlg : public CDialogEx
{
	DECLARE_DYNAMIC(CFacesRebarDlg)
...
public:
	CFacesRebarDlg(ElementHandleCR ehOld, ElementId ehNew, const bvector<ISubEntityPtr>& faces, CWnd* pParent = nullptr);   // 标准构造函数
	CFacesRebarDlg(const vector<ElementHandle>& ehOld, std::vector<vector<EditElementHandle*> >& faces, 
		const map<ElementId, EleFace>& eleFaces, CWnd* pParent = nullptr);
	virtual ~CFacesRebarDlg();
...
// 对话框数据
	...
	enum { IDD = IDD_DIALOG_FacesRebar };
#endif
...
//	void PreviewMultiSlabOrWallRebar(EditElementHandleR eeh, DgnModelRefP modelRef);
...
public:
	int m_CurSelTab;
	CFacesMainRebarDlg					m_PageMainRebar;			//主要配筋
	CDialog*							pDialog[1];					//用来保存对话框对象指针
	virtual BOOL OnInitDialog();
	afx_msg void OnTcnSelchangeTabFacerebar(NMHDR *pNMHDR, LRESULT *pResult);
...
struct          SelectFaceTool2 : LocateSubEntityTool
{
	ElementHandle	_eh;
	ElementId		_ehNew;
	int				_selectNum;
	CFacesRebarDlg *m_ptr;
public:
	...
	* @bsimethod                                                              Bentley Systems
	+---------------+---------------+---------------+---------------+---------------+------*/
	SelectFaceTool2(int cmdName, int prompt, ElementHandleCR ehOld, ElementId ehNew, CFacesRebarDlg *ptr, int selectNum = 1) :_eh(ehOld), _ehNew(ehNew), _selectNum(selectNum), m_ptr(ptr)
	{
...
protected:
	...
	* @bsimethod                                                              Bentley Systems
	...
	virtual void _OnRestartTool() override
	{
		...
		SelectFaceTool2* tool = new SelectFaceTool2(1, 1, _eh, _ehNew, m_ptr);
		tool->InstallTool();
	}
