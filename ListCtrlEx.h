...
namespace ListCtrlEx
{
	...
	struct ButtonCellMsg
	{
		ButtonCellMsg(int nRow = -1, int nCol = -1, BOOL bClicked = TRUE)
			:m_nRow(nRow), m_nColumn(nCol), m_bClicked(bClicked)
		{}
		int m_nRow;
		int m_nColumn;
		BOOL m_bClicked;
	...
	#define WM_ListCtrlEx_BUTTON_LBUTTONDOWN  WM_USER+8010
// User define message 
...
// operations
	public:
		...
		BOOL				GetRowData(int nRow, vector<CString> &oneRowData);
		BOOL				GetAllData(map<int, vector<CString> > &mapAllData);
	protected:
...
// implementation
	private:
		...
		void				DrawButton(CDC *pDC, CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData);
